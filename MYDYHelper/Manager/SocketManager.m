...
+(instancetype)sharedQueue
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (!isExit) {
            ...
            NSInteger time =  date2.integerValue - [dataDictionary[@"lastTime"] integerValue];
            
            if (time > 60 * 20) {
                queue.isfirstLoad = YES;
                queue.lastTime =[dataDictionary[@"lastTime"] intValue];
            }else{
                NSLog(@"isfirstLoad  2221100");
...
- (void)longConnectToSocket
{
    ...
    if (!self.deviectype) {
        ...
        dic =@{@"type":@"ping",@"device_key":self.deviectype?self.deviectype:@"",@"isReload":[NSNumber numberWithBool:self.isfirstLoad],@"onLastTime":[NSNumber numberWithInteger:self.lastTime]};
        if (self.isfirstLoad) {
            NSLog(@"isfirstLoad  22211");
        }
    }
    self.isfirstLoad = NO;
